...
struct
{
  enum
  {
    RETRY_WIFI_CONN_CNT = 3,   // WIFI 杩炴帴閲嶈瘯娆℃暟
    RETRY_MQTT_CONN_CNT = 3,   // MQTT 杩炴帴閲嶈瘯娆℃暟
    RETRY_MQTT_SUBPUB_CNT = 3, // MQTT 璁㈤槄鍙戝竷閲嶈瘯娆℃暟

    LOOP_PERIOD_MS = 50, // 涓诲惊鐜懆鏈?
  ...
  struct
  {
    ...
    // uint32_t wait_response_last_ms; // 涓婃鏃堕棿鎴?
    uint8_t playbuf[32];
  ...
  struct
  {
    uint8_t stat; // 鎸囩ず鑱旂綉鐘舵€佹垨鎸囩ず鍊掕鏃剁姸鎬?
...
void loop()
{
  if (millis() - ctrl_fsm.last_ms_st > ctrl_fsm.LOOP_PERIOD_MS)
  {
    ...
    if (ctrl_fsm.stat == ctrl_fsm.INIT_STAT) // 鏍规嵁宸蹭繚瀛樼殑缁х數鍣ㄧ姸鎬佹帶鍒剁户鐢靛櫒
    {
      // tp();
  ...
  if (millis() - ctrl_fsm.seg_disp_fsm.main_last_ms >= ctrl_fsm.seg_disp_fsm.MAIN_SCAN_PERIOD_MS)
  {
    ...
    else if (ctrl_fsm.seg_disp_fsm.stat == 1) // 鏂綉鏃?,鏁扮爜绠￠棯鐑?"-"鎻愮ず
    {
...
void Key_cb(Key::KeyEvent_et key_event)
{
  ...
  {
    tpf("key long press");
    uint8_t buf[] = {0xbf,0xaa,0xca,0xbc,0xc5,0xe4,0xcd,0xf8};  // 播报,开始配网
